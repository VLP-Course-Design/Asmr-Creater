...
- **变更**: scene_type 走 contracts/scene_type_vocabulary.json(424 值/20 组，none/other_* 兜底)、scene_group 查表生成、secondary_scene_types 最多 2 个、实体收敛为 contracts/anchor_dictionary.json 的 87 个锚点 id(去掉 state)、brightness 改由程序计算(VLM 不再输出)
...
- **遗留**: 87 锚点 bbox/depth/EXIF 检测管线归第二人(docs/VISION_V23_GAP.md)；2.3 契约取舍待团队拍板(ADR-0001 proposed)
